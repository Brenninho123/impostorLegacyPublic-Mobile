...
class FNAFState extends MusicBeatState
{
	...
	override function create():Void
	{
		...
		if (ClientPrefs.fnafHintCode == '')
		{
			var hintCodes = [
				...
				'CHALLENGE', 'SNEEP', 'FUNNI342', 'RUBATO', 'TRIPLETROUBLE', 'STORY', 'NOOB49', 'TRINKETS',
				'DREAMJOB', 'EBK', 'WHITEPARASITE', 'BROIMPOSTOR', 'COMMUNITYGAME', 'PENKARU', 'DEFEAT',
				'LIGHTSDOWN', 'LIGHTSOUT', 'LIGHTSOFF', 'FLIPPY', 'DK', 'HELLSCAPE', 'ZARED'
			];
		...
		if (ClientPrefs.fnafStateVisited)
		{
			...
			FlxTween.tween(dlgBlackScreen, {alpha: 0}, 1.0, {
				...
				onComplete: function(_) { remove(dlgBlackScreen); }
			});
	...
	function applyCameraFilters():Void
	{
	...
	function tweenDepth(target:Float, dur:Float):Void
	{
		if (depthTween != null) depthTween.cancel();
		var from:Float = shaderDepth;
		depthTween = FlxTween.num(from, target, dur, {ease: FlxEase.quadOut}, function(v:Float):Void
		{
			shaderDepth = v;
	...
	function crossfadeMusic(key:String, dur:Float = 1.0, vol:Float = 0.6):Void
	{
		FunkinSound.playMusic(Paths.sound(key), 0);
		FlxTween.tween(FlxG.sound.music, {volume: vol}, dur, {ease: FlxEase.linear});
	}

	function beginLine(lines:Array<String>, idx:Int):Void
	{
		dialogueEnded = false;
		...
		dlgText.completeCallback = function():Void
		{
			dialogueEnded = true;
			if (rtlMode)
			{
	...
	function startPostDialogue():Void
	{
		inPostDialogue    = true;
		dialogueActive    = true;
		postDialogueIndex = 0;
	...
	function advanceDialogue():Void
	{
		...
		FlxTween.tween(dlgBlackScreen, {alpha: 0}, 2.5, {
			ease: FlxEase.quadOut,
			onComplete: function(_) { remove(dlgBlackScreen); crossfadeMusic('amb', 2.5, 0.6); }
		});
		...
		FlxTween.tween(camTarget, {alpha: 1}, 2.5, {
			ease: FlxEase.quadOut,
			onComplete: function(_) { startPostDialogue(); }
		});
	}

	function showMonitorMessage(text:String):Void
	{
	...
	function closeComputer():Void
	{
		...
		FlxTween.tween(FlxG.camera, {zoom: 1}, 0.25, {
			ease: FlxEase.quadOut,
			onComplete: function(_) { screenZooming = false; }
		});
		tweenDepth(4.0, 0.25);
	...
	function returnToComputer():Void
	{
		passwordActive = true;
		vhsOn          = true;
		applyCameraFilters();
	...
	function setComputerUIVisible(vis:Bool):Void
	{
	...
	function layoutPC():Void
	{
		...
		var ix:Float = compPanel.x + compPanel.width  * 0.175;
		var iy:Float = compPanel.y + compPanel.height * 0.22;
		var iw:Float = compPanel.width  * 0.69;
		var ih:Float = compPanel.height * 0.63;

		monX = ix; monY = iy; monW = iw; monH = ih;
	...
	function errorMessage(msg:String):Void
	{
		...
		compStatus.x     = monX;
		var baseX:Float  = compStatus.x;

		if (msg == 'X')
		{
			FlxTween.tween(compStatus, {x: baseX + 10}, 0.03, {
				type: FlxTweenType.PINGPONG,
				onComplete: function(_) { compStatus.x = baseX; }
			});
	...
	function submitCode():Void
	{
		...
		switch (code)
		{
			...
			case 'HELLSCAPE':
				...
				else { crashAfterVideo = true; playVideo('hellscape'); }
				#else
	...
	#if VIDEOS_ALLOWED
	function playFullscreenVideo(key:String):Void
	{
		...
		new FlxTimer().start(0.1, function(_):Void
		{
			if (secretVideo != null) { remove(secretVideo); secretVideo.destroy(); secretVideo = null; }
			...
			secretVideo.onEnd(() -> {
				if (key == 'zared video') { ClientPrefs.scaryZared = true; ClientPrefs.flush(); }
				Sys.exit(0);
			});
			...
			if (loadPath != null && secretVideo.load(loadPath)) secretVideo.delayAndStart();
			else Sys.exit(0);
		});
	}
	...
	function layoutVideo():Void
	{
		...
		var mw:Int = Std.int(monW);
		var mh:Int = Std.int(monH);
		if (mw <= 0 || mh <= 0) return;

		secretVideo.setGraphicSize(mw, 0);
		secretVideo.updateHitbox();
		if (secretVideo.height > monH) { secretVideo.setGraphicSize(0, mh); secretVideo.updateHitbox(); }
	...
	function playVideo(key:String):Void
	{
		if (videoPlaying) return;
		videoPlaying = true;
		...
		new FlxTimer().start(1.0, function(_):Void
		{
			...
			secretVideo.onFormat(() -> { secretVideo.antialiasing = ClientPrefs.globalAntialiasing; layoutVideo(); });
			secretVideo.onEnd(() -> {
	...
	function playAudio(key:String):Void
	{
		...
		new FlxTimer().start(1.0, function(_):Void
		{
			...
			secretAudio = FunkinSound.load(Paths.sound(Paths.sanitize(key)), 1.0, false, null, false, false, null, function():Void
			{
				if (secretAudio != null) { secretAudio.destroy(); secretAudio = null; }
				compMusicImg.visible   = false;
				compMusicLabel.visible = false;
				showLoader();
	...
	function stopAudioAndReturn():Void
	{
		if (!audioPlaying) return;
		if (secretAudio != null) { secretAudio.stop(); secretAudio.destroy(); secretAudio = null; }
		compMusicImg.visible   = false;
	...
	function hideImageAndReturn():Void
	{
	...
	override function update(elapsed:Float):Void
	{
		...
		if (audioPlaying)
		{
			if (FlxG.keys.justPressed.ESCAPE) { FlxG.sound.play(Paths.sound('type')); stopAudioAndReturn(); return; }
			tickVHS();
			return;
		}
		...
		var mx:Float = FlxG.mouse.x / FlxG.width;
		var my:Float = FlxG.mouse.y / FlxG.height;
		var bx:Float = (FlxG.width  - camTarget.width)  * 0.5;
		var by:Float = (FlxG.height - camTarget.height)  * 0.5;
	...
	{
		var hovering:Bool = spr.overlapsPoint(FlxG.mouse.getWorldPosition());
